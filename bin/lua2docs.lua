...
function TLua2DoX_filter.readfile(this, AppStamp, Filename)
    ...
    if inStream:getContents(Filename) then
        ...
        while not (err or inStream:eof()) do
            ...
            elseif string.match(line, '^%s*function%s') then
                inFunctionDoc = true
                ...
                if baseFnName then
                    if currentClass or namespace then
                        local fullNamespace = namespace or currentClass
            ...
            elseif inFunctionDoc == false and string.find(line, "%s*=%s*") then
            ...
            if element.isObject then
                ...
                for _, fn in ipairs(element.functions) do
					if fn.name then
						...
						outStream:writeln("    " .. (fn.returnType or "void") .. " " .. fn.name .. "(" .. (fn.args or "") .. ");")
					end
                end
                outStream:writeln("};")
            end
        ...
        for _, global in ipairs(globals) do
            outStream:writeln("/*! \\fn " .. global.name .. " */")
            ...
            outStream:writeln((global.returnType or "void") .. " " .. global.name .. "(" .. (global.args or "") .. ");")
        end
    else
